...
public class UnoCard {
    ...
    public UnoCard() { // AREN'T WILD AND DRAW 4 NOT SUPPOSED TO HAVE COLOR YET?
        ...
        int randomCard = rand.nextInt(15);
        switch(randomCard) {
            case 0 :
                this.card = CARD.ZERO;
                break;
    ...
    public boolean isPlayable(UnoCard nextCard) {
        if(nextCard.card == CARD.WILD | nextCard.card == CARD.WILD_DRAW4 |
                nextCard.card == card | nextCard.color == color){
            return true;
        }
        return false;
    }
